...
year,month,hour,minute = int(year),int(month),int(hour),int(minute)
...
if not os.path.isdir(result_path2):
...
time = time_bar_create.time_bar()
out = export_file.output_file(result_path,video_path,word_path,max_index,area_index,time,map)
